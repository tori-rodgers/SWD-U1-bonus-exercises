...
console.log(contactInfo.address.street1);

// TODO: Use chained bracket notation to print "Apt. 5C"
console.log(contactInfo['address']['street2']);
...
function printMenu(mealsArray) {
	const line = '-'.repeat(36);
	const newline = '\n';
	console.log(newline + 'MENU');
	for (let meal of mealsArray) {
		console.log(line);
		console.log(`Appetizer: ${meal.appetizer}`);
		console.log(`Entree: ${meal.entree}`);
...
function printFavorites(favorites) {
	...
	const newline = '\n';
	console.log(newline + 'MY FAVORITE THINGS' + newline + line);
	for (let category in entertainmentFavorites) {
		console.log(category.toUpperCase() + ':');
		for (let item of entertainmentFavorites[category]) {
			console.log('  ' + item);
		}
		console.log(line);
	}
...
printFavorites(entertainmentFavorites);
